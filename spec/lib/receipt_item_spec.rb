...
RSpec.describe ReceiptItem do
  describe '#final_price' do
    it 'calculates the correct final price', :aggregate_failures do
      item = ReceiptItem.new(quantity: 1, product_name: 'music CD', price: BigDecimal('14.99'))
      expect(item.final_price).to eq(BigDecimal('16.49'))

      item = ReceiptItem.new(quantity: 2, product_name: 'book', price: BigDecimal('12.49'))
      expect(item.final_price).to eq(BigDecimal('24.98'))

      item = ReceiptItem.new(quantity: 1, product_name: 'imported bottle of perfume', price: BigDecimal('47.50'))
      expect(item.final_price).to eq(BigDecimal('54.65'))
    end

    it 'handles multiple quantities correctly' do
      item = ReceiptItem.new(quantity: 3, product_name: 'imported boxes of chocolates', price: BigDecimal('11.25'))
      expect(item.final_price).to eq(BigDecimal('35.55'))
    end

    it 'handles zero tax correctly' do
      item = ReceiptItem.new(quantity: 1, product_name: 'chocolate bar', price: BigDecimal('0.85'))
  ...
  describe '#total_tax' do
    ...
    it 'calculates zero tax for exempt items', :aggregate_failures do
      ...
      item = ReceiptItem.new(quantity: 1, product_name: 'packet of headache pills', price: BigDecimal('9.75'))
      expect(item.total_tax).to eq(BigDecimal('0.00'))
    end
    ...
    it 'handles rounding correctly', :aggregate_failures do
      item = ReceiptItem.new(quantity: 1, product_name: 'imported something', price: BigDecimal('1.01'))
